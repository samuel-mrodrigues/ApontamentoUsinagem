...
/**
 * Retorna uma classe que manipula um arquivo JSON
 * @param {String} arquivoCaminho Diretorio do arquivo de forma local
 ** O diretorio inicial é a raiz onde o programa está instalado, sendo assim, um exemplo passar como parametro a seguinte string "/logs/logs.txt", irá ser por exemplo (C:/Users/AppData/MeuPrograma)/logs/logs.txt
 * @returns {Promise<ArquivoJSON>} Uma classe arquivo do tipo JSON para manipulação do mesmo
 */
export async function getArquivoJSON(arquivoCaminho) {
    let diretorioArquivo = path.resolve(arquivoCaminho);

    return new ArquivoJSON(diretorioArquivo)
}

/**
 * Retorna uma classe que manipula um arquivo TXT
 * @param {String} arquivoCaminho Diretorio do arquivo de forma local
 ** O diretorio inicial é a raiz onde o programa está instalado, sendo assim, um exemplo passar como parametro a seguinte string "/logs/logs.txt", irá ser por exemplo (C:/Users/AppData/MeuPrograma)/logs/logs.txt
 * @returns {Promise<ArquivoTXT>} Uma classe arquivo do tipo JSON para manipulação do mesmo
 */
export async function getArquivoTXT(arquivoCaminho) {
    let diretorioArquivo = path.resolve(arquivoCaminho);

    return new ArquivoTXT(diretorioArquivo)
}
...
export class ArquivoJSON extends Arquivo {
    ...
    setConteudo(objeto) {
        ...
        let conteudoString = '';
        try {
            conteudoString = JSON.stringify(objeto, null, 1);
        } catch (ex) {
            console.log(ex);
        ...
        if (Array.isArray(conteudoEscrever)) {
            ...
            conteudoEscrever.forEach(conteudoArray => {
                if (typeof conteudoArray == "object") {
                    ...
                    try {
                        linhasParaEscrever.push(JSON.stringify(conteudoArray, null, 1));
                    } catch (ex) {
                        console.log(ex);
            ...
            try {
                linhasParaEscrever.push(JSON.stringify(conteudoEscrever, null, 1));
            } catch (ex) {
...
function criarDiretoriosInexistentes(diretorioAlvo) {
    ...
    for (const diretorio of diretoriosEncontrados) {
        ...
        // Se for um arquivo, ignora
        if (diretorio.indexOf(".") != -1) break;
